    ...
    public class LatestVideosProvider : ILatestVideosProvider
    {
        ...
        private void FetchLatestVideo(string youtubeChannelId)
        {
            var items = this.videosFetcher.GetAllVideosFromChannel(
                youtubeChannelId,
                x =>
                x.Snippet.Description.ToLower().Contains("ivanntakov") || x.Snippet.Description.ToLower().Contains("takov")
                || x.Snippet.Title.ToLower().Contains("ivan") || x.Snippet.Title.ToLower().Contains("ivann"));
